...
function App() {
  ...
  const [pageId, { refetch }] = createResource(
    () => tabInfo()?.url,
    checkIfPageSaved
  );
  ...
  async function handleSavePage() {
    ...
    try {
      sendMessage("savePage", {
        title: tabInfo()!.title,
        url: tabInfo()!.url,
        description: note() ?? tabInfo()!.description,
      }).finally(refetch);
    } catch (error) {
  ...
  async function handleUnsavePage() {
    ...
    try {
      await sendMessage("deletePage", { pageId: pageId()!.id }).finally(
        refetch
      );
    } catch (error) {
    ...
    <div class="flex flex-col border border-gray-300 w-[280px] p-4 m-0 bg-white font-sans">
      <nav class="mb-4 flex justify-between items-center">
        <div class="flex gap-1 items-center">
          <span class="font-serif text-sm italic">Finest</span>
          <button
      ...
      >
        <main>
          <div class="mb-4">
            ...
            <p class="italic text-xs m-0 truncate" title={tabInfo()?.url}>
              {tabInfo()?.url}
            </p>
          </div>

          <div class="mb-4">
            <label for="note" class="block mb-1 text-xs tracking-wider">
              Note
            </label>
            <textarea
              id="note"
