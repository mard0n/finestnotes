...
const highlight = new Hono<{ Bindings: Bindings, Variables: { user: User; session: Session } }>()
  ...
  .post("/", protect, zValidator("json", z.object({
    ...
    let page = await db.select().from(pages).where(and(eq(pages.url, pageURL), eq(pages.userId, c.var.user.id))).get();
  ...
  .delete("/:id", protect, zValidator("param", z.object({
    ...
    const res = await db.delete(highlights).where(and(
      eq(highlights.id, Number(id)),
      eq(highlights.userId, c.var.user.id),
    )).run();
